...
| Uncommitted | `sizing.py`, `costs.py`, `metrics.py`, `backtest.py`, `ma_cross.py`, `signals/__init__.py`, `run_gauntlet.py`, `CONCEPTS.md`, `CLAUDE.md`, log entries 19-24, DECISIONS.md ledger, graveyard report |
| Functions implemented | 21 of 41 (plus helpers) |
| Tests | 23 passing, 1 failing (roll adjustment, still a stub) |
| Linter | clean |
| Real results produced | `reports/graveyard.md`: 3 of 8 signals judged, **0 survived**, all on condition 2 |
...
## Entry 24, 2026-08-21, The graveyard table exists

`reports/graveyard.md` is generated and committed. Three signals judged, none survived.

Two changes from the Entry 23 version, both to match the specification.

**Cause strings are condition-numbered and uniform.** They had been descriptive and
embedded the failing value, so every row read differently. Now every cause is one of
three fixed strings naming a condition. In a table whose purpose is comparison, uniform
causes group; bespoke ones do not.

**Added a Cause of death column, and gross OOS.** The table had neither. Cause of death
is the first column a reader scans in a document called a graveyard, and its absence was
...
### What the gross column revealed

**Transaction costs are not the binding constraint for any of these signals.**

```
              gross    net    @4bp    gap
ma_cross      0.527  0.523   0.518  0.004
ts_momentum   0.517  0.507   0.496  0.010
seasonality   0.317  0.310   0.303  0.007
```

The gap between gross and net is under a hundredth of a Sharpe in every case, and
doubling the cost assumption to 4 bp changes no verdict. **Set costs to zero and all
three still die.**

That is worth stating plainly because it identifies the cause of death precisely. These
are not edges consumed by fees, which would be a story about implementation. They are
patterns that cannot be distinguished from selection across eight candidates, which is a
story about the pattern not being there. All three fail on condition 2, and none is
close: 0.503, 0.482, 0.289 against a bar of 0.95.

### Detail visible in the momentum autopsy

`ts_momentum` is negative in two regimes, `range_recov` at Sharpe -0.474 and `bear_2013`
at -0.032, giving it the weakest condition 3 pass of the three at 3 of 5. Its 12 month
lookback did worst precisely in the range-bound stretch, which is the failure mode trend
following is known for and which the regime table surfaces without being asked.

### Status of the deliverable

The five pieces named in the project spec, assessed honestly:

| Deliverable | State |
|---|---|
| `graveyard.md` headline table | exists, 3 of 8 signals |
| Per-signal autopsies | exists, with pre-stated hypothesis and regime breakdown |
| The overfitting demo | not started |
| The reusable backtest engine | working, documented, adversarially tested |
| One-page summary | not started |

The framework is complete end to end. What remains is coverage, five signals, and the
two correctness items that sit underneath every number: the roll adjustment and the
corrupt-bar decision.
